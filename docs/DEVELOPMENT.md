...
- **GitHub release builds** regenerate the full release history from git tags using `conventional-changelog` and embed it as `src/Onpa/Resources/CHANGELOG.md` immediately before `xcodebuild archive`. This is necessary because `@semantic-release/changelog` only writes the *new* release into the working-tree `CHANGELOG.md` and the file is intentionally not committed back to the repo, so the file passed in by semantic-release contains a single section. Regenerating from `git tag` history ensures the in-app changelog shows every previous release. The unreleased section emitted by `conventional-changelog` (labeled with the placeholder `0.0.0-development` from `package.json`) is rewritten to the real release version that semantic-release passed in.
- If the regeneration fails for any reason, the build script falls back to copying the single-release `CHANGELOG.md` produced by `@semantic-release/changelog` so the build still ships *something*.
